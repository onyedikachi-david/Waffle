...
/**
 * Saves and clears an MWA connection to an on-device wallet, with convenience value
...
class WalletConnectionUseCase @Inject constructor(
    ...
    val walletDetails = combine(dataStoreRepository.publicKeyFlow, dataStoreRepository.accountLabelFlow, dataStoreRepository.authTokenFlow) {
        pubKey, label, authToken -> if (pubKey.isEmpty() || label.isEmpty() || authToken.isEmpty()){NotConnected} else {
            Connected(publicKey = pubKey, accountLabel = label, authToken=authToken)
    }
    }
