...
class PrefsDataStoreRepository @Inject constructor(@ApplicationContext private val context: Context) {
    val publicKeyFlow: Flow<String> = context.dataStore.data.map {
        preferences -> preferences[pubkeyPref] ?: ""
    }

    val accountLabelFlow: Flow<String> = context.dataStore.data.map { preferences -> preferences[accountLabelPref] ?: "" }

    val authTokenFlow: Flow<String> = context.dataStore.data.map { preferences -> preferences[authTokenPref] ?: "" }
